...
import json   #这段代码用来修改print_metrics 改成保存到json 方便五折交叉验证
def print_metrics_to_json(metrics, save_to_path=None):
    # 准备 JSON 数据
    json_data = {}
    for key in sorted(metrics.keys()):
        json_data[key] = {}
        for v in metrics[key]:
            cur_key = v[0]  # 子指标名称，如 "train" 或 "test"
            value = v[1]    # 子指标值
            if isinstance(value, list):
                # 如果值是列表，保存原始列表、均值和标准差
                json_data[key][cur_key] = {
                    "values": value,
